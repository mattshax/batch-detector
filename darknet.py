...
Modified by Matthew Shaxted - 20200603
"""
...
def performDetect(imagePath="data/dog.jpg", thresh= 0.25, configPath = "./cfg/yolov4.cfg", weightPath = "yolov4.weights", metaPath= "./cfg/coco.data", showImage= True, makeImageOnly = False, initOnly= False, outImage = False, includeClasses=[]):
    """
    ...
    if showImage:
        #try:
            ...
            for detection in detections:
                ...
                if len(includeClasses) == 0 or label in includeClasses:
                    ...
                    # try adding labels here
                    #print(draw)
                    
                    # from PIL import Image, ImageDraw, ImageFont
                    # drawAll = ImageDraw.Draw(image)
                    # drawAll.save(outImage, optimize=True)
            ...
            if not makeImageOnly:
                ...
                io.imsave(fname=outImage, arr=image)
                ...
                for detection in detections:
                    ...
                    if len(includeClasses) == 0 or label in includeClasses:
                        ...
                        bounds = detection[2]
                        boxColor = (int(255 * (1 - (confidence ** 2))), int(255 * (confidence ** 2)), 0)
                        color = 'rgb('+str(boxColor[0])+', '+str(boxColor[1])+', '+str(boxColor[2])+')'
                        yExtent = int(bounds[3])
                        xEntent = int(bounds[2])
                        # Coordinates are around the center
                        xCoord = int(bounds[0] - bounds[2]/2)
                        yCoord = int(bounds[1] - bounds[3]/2)
                        drawAll.text((xCoord, yCoord-size-2), pstring, fill=color, font=font)
                    
                image.save(outImage, optimize=True)
...
def performBatchDetect(thresh= 0.25, configPath = "./cfg/yolov4.cfg", weightPath = "yolov4.weights", metaPath= "./cfg/coco.data", hier_thresh=.5, nms=.45, batch_size=3, img_samples=[]):
    import cv2
    import numpy as np
